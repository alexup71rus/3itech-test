...
const decode = input => input
    .replace(/((\.?\-?)[0-9]+)/g, x =>
        Array(+Array.from(x).slice(1).join(''))
            .fill(x[0] === '.' ? 1 : 0).join('')
    )
    .replace(/(([01])\2*)/g, x => x.length === 1 ? '.' : String(Math.ceil(-(1 - x.length / 2))) )
    .match(/(\d+)\.(\d+)/g)
    .map(x => x.split('.'))
    .sort((x, y) => +x[1] - +y[1])
